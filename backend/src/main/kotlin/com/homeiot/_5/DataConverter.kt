...
@Component
class DataConverter {
    val RESISTIVE_HIGH = 4095.0f
    val RESISTIVE_LOW = 2300.0f
    val CAPACITIVE_1_HIGH = 4095.0f
    val CAPACITIVE_1_LOW = 1500.0f
    val CAPACITIVE_2_HIGH = 4095.0f
    val CAPACITIVE_2_LOW = 2600.0f

    private fun convertReading(reading: Float, highLimit: Float, lowLimit: Float): Float {
        return BigDecimal(((1.0f - (reading - lowLimit) / (highLimit - lowLimit)) * 100.0f).toString()).setScale(
            2,
    ...
    fun convert(data: Data): Data {
        return Data(
            ...
            data.airTemperature,
            data.brightness,
            convertReading(data.soilHumidity1, RESISTIVE_HIGH, RESISTIVE_LOW),
